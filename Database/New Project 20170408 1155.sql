...
INSERT INTO `bookrenewal` (`renewalId`,`bookIssueId`,`status`,`dateTime`,`extendedDateTime`) VALUES 
 ...
 (5,2,'REQUESTED','2017-04-07 17:39:53','2017-01-16 14:22:51'),
 (6,2,'REQUESTED','2017-04-07 17:55:54','2017-01-18 14:22:51'),
 (7,2,'REQUESTED','2017-04-07 17:56:11','2017-01-20 14:22:51'),
 (8,2,'REQUESTED','2017-04-07 17:56:40','2017-01-22 14:22:51'),
 (9,2,'REQUESTED','2017-04-08 11:47:26','2017-01-24 14:22:51');
...
INSERT INTO `fine` (`fineId`,`bookIssueId`,`fineAmount`,`dateTime`,`comment`) VALUES 
 ...
 (4,2,300,'2017-04-07 18:05:55','fine');
...
--
-- Procedure `booklibrary`.`getFine`
--

DROP PROCEDURE IF EXISTS `getFine`;
DELIMITER $$

CREATE DEFINER=`root`@`localhost` PROCEDURE `getFine`(IN p_bookIssueId	INT)
BEGIN
  SELECT DISTINCT
    B.title                                        AS title,
    F.fineAmount                                   AS fineAmount,
  ...
  INNER JOIN
    bookissue                                      AS BC
  ON
      F.bookIssueId = BC.bookIssueId
  INNER JOIN
    book                                           AS B
  ON
    BC.bookId = B.bookId
  WHERE
    BC.bookIssueId = p_bookIssueId
  ORDER BY
    F.dateTime ASC;
END $$
...
BEGIN
    ...
    THEN
		...
		FROM bookIssue WHERE userId = p_userId AND  bookIssueId = p_bookIssueId;
